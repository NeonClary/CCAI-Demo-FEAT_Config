...
const sectionIcons = {
  action_items: Target,
  project_plans: TrendingUp,
  safety_compliance: AlertTriangle,
  cost_budget: BarChart3,
  scheduling: Calendar,
  quality_notes: BookOpen,
  equipment_logistics: Users,
  environmental: Heart,
  challenges_obstacles: AlertTriangle,
  key_decisions: Lightbulb
};
...
const CanvasPage = ({ user, authToken, onNavigateToChat, onSignOut }) => {
  ...
  const appName = config?.app?.title || config?.app_settings?.app_name || 'Construction Advisor Panel';
    ...
    <div className={`canvas-page ${isPrintView ? 'print-view' : ''}`}>
      ...
      <div className="canvas-header">
        <div className="header-left">
          ...
          <div className="canvas-title-section">
            ...
            <p className="canvas-subtitle">Action items, plans, and notes at a glance</p>
          </div>
        </div>
        ...
        {sortedSections.length === 0 ? (
          <div className="empty-canvas">
            ...
            {isUpdating || isProcessingFirstTime ? (
              ...
              <div>
                <p>Start chatting with your AI advisors to populate your notes and plans with insights!</p>
      ...
      {isPrintView && (
        <div className="print-footer">
          ...
          <p>User: {user?.email} | Total Insights: {canvasData?.total_insights || 0}</p>
        </div>
      )}
    </div>
  );
